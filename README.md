...
- `npm test` runs [tests/e2e.mjs](tests/e2e.mjs) — **221 checks** (the suite's last check verifies this number against itself) plus a 17-fixture parser-conformance suite and a 10-row ledger-grammar fence. CI runs it on every push, on Linux and macOS.
