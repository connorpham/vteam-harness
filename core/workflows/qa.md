...
args: "<TICKET: {project.key}-nnn | tracker URL> [assignee=<name>] [base=<app URL — overrides config app.url for this run>]"
---
...
- **UI EXPECTED (from the design oracle)** — ① design link on the ticket:
  ...
  → READ it: re-run `node .vteam/profiles/{stack.profile}/scripts/ui_fidelity.mjs <TICKET>` to confirm the
...
`{paths.team}/competencies/qa/INDEX.md` (read the index, not the tree): **every
...
`{paths.evidence}/<TICKET>/TC_<n>/journey.mjs` importing `launch`/`shot` from
...
python3 .vteam/scripts/annotate.py box \
  --img {paths.evidence}/<TICKET>/TC_<n>/03_result.png --rect X,Y,W,H [--rect ...] \
...
1. Finalize REPORT.md (challenger's readability fixes applied). Re-run
   ...
   ```bash
   python3 .vteam/scripts/evd_index.py  --evd {paths.evidence}/<TICKET>            # index block → manifest.md
   python3 .vteam/scripts/xlsx_export.py --evd {paths.evidence}/<TICKET> --strict  # <TICKET>_testcases.xlsx
   ```
