...
**Who is working:** read `{paths.team}/competencies/dev/dev-identity.md` first —
the senior engineer this lane runs as (decision posture, what a senior never
does) — then `{paths.team}/roles/dev.md`, the professional baseline. The lane
...
`{paths.team}/competencies/dev/INDEX.md` (read the index, not the tree): every
...
`{paths.evidence}/<TICKET>/dev/REPORT.bdd.md` — what you built as
Given/When/Then scenarios in plain words (`{paths.team}/bdd-report.md`;
