...
name: ba
command: /ba
description: BA pipeline — turns the upstream spec into a runnable backlog that /dev and /qa can execute. Reads the spec + schema + existing backlog → shards the spec into {paths.specs}/<feature>.md (index + per-feature files, the repo's citable oracle) → decomposes a feature into user stories with TESTABLE acceptance criteria (Given/When/Then, each tracing to a spec section) → flags every gap/contradiction as a question for the owner instead of inventing requirements → challenger review → creates the tickets per the B4 gate (owner approves the draft when present; in owner-absent sessions at full autonomy, challenger APPROVE suffices; dry-run always stops at the draft).
args: "<feature|spec section|'all'> [epic=<epic key>] [dry-run]"
---
...
4. **Dedup before create.** Check the existing backlog and
   `{paths.qa}/known-issues.md` before drafting — a story that already exists
...
- **Role playbook**: read `{paths.team}/roles/ba.md` first — the thinking floor
  ...
  measured by.
- **KB preflight (index-only)**: `{paths.qa}/knowledge-base.md` §0 + INDEX, open
  lessons tagged `ba`/tracker/this feature's area; `{paths.qa}/known-issues.md`
  headings.
...
- **Decompose** into stories per template (one block per story in the draft):
  ...
  - **Spec:** §x.y ({paths.specs}/<feature>.md)
...
- Learning loop: new lesson (+1 INDEX line) into `{paths.qa}/knowledge-base.md`
  or "no new lesson"; prefer GRADUATION per §0.

## Definition of Done
- [ ] Preflight ran; scope announced
- [ ] Verbatim spec shard lives in `{paths.specs}/` + INDEX updated (if new)
