# vteam — the full guide

Everything the front page links to, in one place: transcripts, workflows, gates, the evidence pack, configuration, the command reference, known limits. The front page (README) is the ten-second version.
...
|---|---|
| **A gate that has never been red does not exist** | Every checking gate ships a `--selftest` mutation proof: feed it a violating input, watch it fail. `doctor` discovers every selftest-bearing check and runs them all (34 today)
. An always-green check gets fixed or deleted. |
...
- **`app:` (start / url / health / open_files / headed)** — the runnable app the dev/QA lanes drive. With it set, env bring-up is a quotable proof (`app_check.sh` → `APP: UP`), QA journeys open a **real Chrome window** through `browser.mjs` (scripts kept as re-runnable evidence), and `/dev` opens the files it edits in your editor (`open_files.sh`, Cursor/VS Code auto-detected). Empty on repos with no web app; `headed: never` keeps unattended shifts windowless without dropping a single screenshot. A repo whose `package.json` declares `next` gets this block prefilled by `init`: `start: npx next dev -p <port>`, `url: http://127.0.0.1:<port>`, `health: /`, with the port derived from the checkout's absolute path so two clones or worktrees on one machine never share one (benchmark findings E2/E7/E15, 2026-09-03: an empty block left the lanes with nothing to drive, and two arms on `:3000` had the gate measure each other's server).
...
<picture>
  <img src="https://raw.githubusercontent.com/connorpham/vteam-harness/main/docs/assets/commands.svg" alt="Every vteam command end to end: audit grades with no install, init writes config, gates, skills and fence, doctor proves the install with 34 selftests
. Daily: the agent tool runs the workday, 18 gates can refuse with exit 1, everything lands in committed files; board and graph mirror those files and never fail the build. resume derives where a dead session stopped; usage measures who ran which model at what cost; update refreshes framework files by manifest hash." width="100%">
</picture>
...
Reads everything init wrote and **runs the proof**: python present, config parses in all three parser languages, every manifest hash intact (files someone edited are *named*, not counted), hooks wired, then all 34 discovered selftests
 — each one feeds its gate a violating fixture and demands RED — then pings the tracker, git remote and hosting CLI for real. From this repository:
...
✅ gate selftests green (34 discovered checks prove they can red)
...
Working, and the proof ships with it: `npm test` runs [tests/e2e.mjs](https://github.com/connorpham/vteam-harness/blob/main/tests/e2e.mjs) — **232 checks**
