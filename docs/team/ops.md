...
(`docs/pm/log.md`), the decision queue (`docs/pm/decisions.md`). A session that
...
`docs/pm/acceptance.md`: every machine-provisional decision, a demo checklist per
...
| Knowledge-base graduation | the KB's own §0 (`docs/qa/knowledge-base.md`) |
...
`docs/pm/hatch-log.md`. The secret scan runs before every hatch and has none.
...
| Sprint planning | First day of each sprint: PM reconciles the plan (`docs/pm/plan.yaml`) with reality — velocity, open change requests, capacity — and logs the delta in session minutes; drift >1 person-day updates the plan | Sprint start | Yes |
| Backlog refinement | BA lane dispatched when the next sprint lacks ready tickets | Mid-sprint | Yes |
| Review / demo | Acceptance dossier (`docs/pm/acceptance.md` demo checklist + sign-off log) | When backlog drains | Yes |
...
- **`tok ≈` is self-reported; `vteam usage` is measured.** Each member runs
  ...
  writes `docs/pm/usage/<actor>.md` (one file per person = conflict-free).
