...
| **External reviewer cards** (`review.external.<id>` — e.g. codex holding R2) | model comes from that config block, NOT this table | cross-model diversity is the point; the tier ladder governs Claude seats only |
...
3. **Record the model in the ledger:** DEV rows in `docs/pm/log.md` note the tier
