...
| In Progress | DEV | On pickup, **with a claim comment `claimed <timestamp> · <branch>`** — In Progress is a CLAIM: other sessions seeing a claim under the claim TTL (**`team.claim_ttl_hours`, default 2 — the config is the one home of this number**; machine-read by `vteam resume`, prose-enforced by the PM recovery lane) keep off; a claim past TTL with no remote branch/worklog is orphaned and handled by the PM recovery lane |
