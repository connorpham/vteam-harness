...
# least privilege: the gate only READS the repo. Actions are pinned by full
# SHA (the tag is a comment) — tags are mutable, a repo's supply chain should
# not hinge on one. Enable Dependabot for github-actions to keep pins fresh.
permissions:
  contents: read
jobs:
  gate:
...
/** Append-only shared files that get `merge=union` (two humans, zero conflicts).
 * decisions.md is deliberately NOT here — see the comment at the write site. */
export const GITATTR_RULES = [
  "docs/pm/log.md merge=union",
  "docs/pm/hatch-log.md merge=union",
  "docs/qa/knowledge-base.md merge=union",
...
export const GITIGNORE_RULES = [
  ...
  ".vteam/doctor.json",
  "# vteam: python bytecode regenerated by the gates — never commit, never manifest",
  ".vteam/**/__pycache__/",
  "# vteam: machine-local code-map index (rebuild anytime: code_map.py build)",
  ".vteam/map.json",
];
...
`;
  ...
  // append-only shared files: union merge keeps two humans' parallel appends
  // conflict-free. decisions.md is DELIBERATELY absent — its status cells are
  // edited in place, and a union merge would keep both sides of a conflicting
  // edit; conflicts there are real disagreements a human must resolve.
  const missing = GITATTR_RULES.filter((r) => !gaHave.has(r));
  if (missing.length) {
    fs.appendFileSync(ga, (gaText && !gaText.endsWith("\n") ? "\n" : "") +
      "# vteam: append-only shared files — union merge keeps two humans' appends conflict-free\n" +
      missing.join("\n") + "\n");
  }
