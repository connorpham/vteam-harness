...
argument-hint: "[n=N item cap | full: run until dry | no-decide: skip the asking step | lite: no background lanes, cheapest]"
---
...
config `autonomy.self_merge` (this install: **false**; off =
...
**Role playbooks** (`docs/team/roles/`): the main loop reads `pm.md`; every
...
- **Parallel (`team.parallel: N > 1`) — fan out the coding, serialize the
  ...
  in its **own git worktree** on the transport in `docs/team/parallel-transport.md`
...
**Session minutes** (`docs/pm/sessions/YYYY-MM-DD-hhmm.md`; two sessions
...
- Exchange: <≤5 SUMMARY lines of the review/challenge rounds — conclusions, no transcripts>
  → details: evd/<KEY>/dev/review.md · docs/specs/reviews/… · docs/adr/…
...
**Bookkeeping commits IMMEDIATELY after writing:** every `docs/pm/` update
...
1. **A spawn brief = paths, not content.** A child agent's brief contains only:
   ...
   PR's diff`, `evd/<KEY>/`). Pasting long spec/diff/document text
...
hoped-for one — **`docs/team/parallel-transport.md` is the wire**. On Claude
...
`docs/pm/coordination.md`. The agents coordinate for exactly two reasons: to
...
- [ ] Session minutes exist in `docs/pm/sessions/` — one block per item,
      summary + links
