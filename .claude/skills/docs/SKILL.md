...
3. **ONE interview round.** All questions go to the owner in a single batched
   table (decision-queue discipline, `docs/pm/decisions.md`). Follow-ups born
...
**Deliverable — the appearance map** (`docs/specs/DRAFT-system-map.md`, and
...
- **One round.** Ask everything at once. An answer that raises a new question →
  a row in `docs/pm/decisions.md` with a proposal and what it blocks, NOT a
  second interview.
...
- **Record the answers verbatim** into
  `docs/specs/DRAFT-system-map.md` under `## Owner answers <date>` before
...
1. **`docs/specs/INDEX.md`** — one row per area (area | file | scope | status
...
2. **Per-area spec shards `docs/specs/<area>.md`** — for each area in the map:
...
3. **`docs/pm/decisions.md` seeds** — the architectural choices the code
...
4. **`docs/qa/knowledge-base.md` seeds** — §0 + INDEX per the standard shape,
...
5. **`docs/qa/known-issues.md`** — one heading per landmine from Q6, each with:
...
5. **Close the learning loop** — append a lesson (+1 INDEX line) to
   `docs/qa/knowledge-base.md` or state "no new lesson".
...
- [ ] Owner answers recorded verbatim before any doc was written; follow-ups went
      to `docs/pm/decisions.md`, not to a second interview round
- [ ] `docs/specs/INDEX.md` + one shard per area exist; every `§` carries
