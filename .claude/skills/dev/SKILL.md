...
argument-hint: "<TICKET: VT-nnn | tracker URL> [assignee=<name>] [branch=feat|fix]"
---
...
for `docs/qa/known-issues.md` headings. Over the cleanup threshold (§0) →
...
Write `evd/<TICKET>/dev/tasksheet.md` BEFORE touching code (it
...
- **`CODE-SCOPE:`** one line, the paths this ticket may touch (e.g.
  ...
  - **Bookkeeping homes are never CODE-SCOPE.** `docs/pm` (coordination log,
    ledger, minutes), `evd` (your own evd/) and `docs/qa`
  ...
  - **In parallel mode (`team.parallel` > 1) COMMIT the tasksheet FIRST**,
    ...
    `docs/team/parallel-transport.md`.
...
- **UI work — design from the design source, consistency from the design
  ...
  2. Read `docs/design/design-language.md` (tokens generated from the
...
- **UI change → PROPER EVIDENCE, machine-guarded:**
  1. Bring the app up (Environment block: `app.start` in a background terminal,
     ...
     `node .vteam/profiles/generic/scripts/ui-evidence.mjs` — it signs
     ...
     **HEADED by default** — the browser visibly opens and walks each screen at
     ...
     `evd/<TICKET>/dev/` — the root layer belongs to QA; two lanes,
...
- **API/backend change → its own evidence standard, not "some curl":**
  `evd/<TICKET>/dev/api.md` with one REAL request→response pair per
...
2. **UI ticket — measure fidelity by machine, not by eye** (proportionality: a
   ...
   - `fidelity.json` — key elements' selectors + expected values **taken from the
     ...
     self-grading).
   - `node .vteam/profiles/generic/scripts/ui_fidelity.mjs <TICKET>` → generates `fidelity.md`.
...
`evd/<TICKET>/dev/review.md` — this file COMMITS with the code, and
...
The runner pipes the brief (`docs/team/review-standard.md` verbatim + the card
...
5. **Close the learning loop** — append a lesson (+ one INDEX line) to
   `docs/qa/knowledge-base.md` or state "no new lesson". Prefer GRADUATION
...
1. **Leave the tree explainable** — either commit WIP with a `wip(<TICKET>): <what is
   ...
   hook). It writes `evd/<TICKET>/dev/STOP-STATE.md` — branch, HEAD, the
...
- [ ] T0: `dor_check.py <TICKET>` GREEN (or a waiver commented on the ticket —
      ...
      before coding
- [ ] `evd/<TICKET>/dev/tasksheet.md` exists with spec/schema
