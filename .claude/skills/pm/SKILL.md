...
argument-hint: "[status | next | run [n=N] | decide | sa <ADR topic>]"
---
...
| **SA** | architecture decisions → ADRs in `docs/adr/` | the SA lane below (agent + challenger) |
...
4. **Everything needing the owner funnels to ONE place:** the decision queue
   (`docs/pm/decisions.md`) + the "Your desk" table at session end. The owner
   never has to read the tracker/PRs/logs to know what they must do.
5. **The ledger:** every dispatched item writes 1 row to `docs/pm/log.md`
...
2. **Role playbook:** read `docs/team/roles/pm.md` — the thinking floor
   (empiricism, low WIP, unblock before adding, quality is not an adjustment
   variable) this loop is measured by.
3. Read: the decision queue · the sprint plan (`docs/pm/plan.yaml`) · the
...
2. **DEV** — the next unblocked item, in sprint-plan order (the order means
   ...
   each as a worktree agent over the transport in
   `docs/team/parallel-transport.md` (Claude Code: the Orca Run mailbox via
   `bash .vteam/scripts/orca_team.sh open-run …`; else the text-relay fallback) —
   hand each one how to reach the others + the `docs/pm/coordination.md` path
   so they can split scope / hand off a shared contract (`coord_check` keeps those
   decisions honest) — and integrate their PRs serially (see /team T2), re-running
   `parallel_check` + `coord_check` after any handoff. Items that would touch a
   file another in-flight branch owns
   wait their turn.
...
1. Spawn 1 fresh agent (model `workhorse`; the challenger too) reading:
   **`docs/team/roles/sa.md` (mandatory)** + the originating question (full
   text from the backlog draft) + the relevant spec shard + the schema + the
   owner's answer if one exists → writes `docs/adr/ADR-nnn-<slug>.md`:
...
0b. **Learn the design language before drawing**: refresh/verify
   `docs/design/design-language.md` (palette/type-scale/radii/components
...
1. Spawn 1 fresh agent (`standard`): reads the screen's spec shard + the
   ...
   `docs/design/mockups/<Screen>-<slug>.html`, plus a 5-line note: which
...
4. **The change ledger (credit: OpenSpec):** an answer that CHANGES or ADDS a
   ...
   in `docs/specs/changes.md`: source (question id), the requirement delta in
...
2b. **The acceptance dossier:** when the current sprint's backlog DRAINS (every
   ticket Done or exemption-blocked) → write/update `docs/pm/acceptance.md`:
...
6. **Sync measured usage** — run `npx vteam-harness usage --sync` and commit the
   `docs/pm/usage/<actor>.md` it writes. This publishes MEASURED numbers from
...
- [ ] New lesson → `docs/qa/knowledge-base.md` per its §0, or "no new lesson"
