...
**Who is verifying:** read `docs/team/competencies/qa/qa-identity.md` first —
the senior tester this lane runs as (the oracle rule, severity by consequence,
the third outcome) — then `docs/team/roles/qa.md`. Immediately after, read
`docs/team/competencies/qa/qa-requirement-smells.md` and run its three-line
...
**KB preflight (index-only):** `docs/qa/knowledge-base.md` §0 + INDEX BEFORE
designing anything; open ONLY tag-matching lessons and answer them in the
verify-sheet (cite by TITLE). Scan `docs/qa/known-issues.md` headings — any
divergence found later is deduped against it before being called a bug.

**Read the change ledger BEFORE the shard:** `docs/specs/changes.md` — filter
CH-nn rows touching this ticket's shard/screen. An expected value that ignores a
decided CH is a wrong expected value.

Write `evd/<TICKET>/verifysheet.md` (REPORT.md points to it in the
...
- **UI EXPECTED (from the design oracle)** — ① design link on the ticket:
  download frames to `evd/<TICKET>/design/`, exact values from node
...
Plan in the verify-sheet — usually 2–5 TCs. **Every TC carries a `TITLE:` (a
sentence about behaviour — "A quantity of 0 is refused", never "test qty") and a
`KIND:` from the six the gate knows** — `acceptance` · `boundary` ·
`whole-screen` · `write-readback` · `exploratory` · `security`; the pack MUST
hold a `boundary` and a `whole-screen` case, and the folder is named
`TC_<n>_<what_it_proves>` so `ls` reads as a test plan:
- ① **Exact acceptance path** (`KIND: acceptance`) — the criterion as the ticket/spec states it.
...
- **Write gate (principle 4): user present → ask first, one block per write**
  ...
  `evd/<TICKET>/data_prep/`, mark rows `ZZTEST`, verify by read-only
...
python3 .vteam/scripts/annotate.py box \
  --img evd/<TICKET>/TC_<n>/03_result.png --rect X,Y,W,H [--rect ...] \
  --label "TC_<n>: <what this proves / what diverges>" \
  --out evd/<TICKET>/TC_<n>/03_result_boxed.png
```

The box **hugs exactly X,Y,W,H** (a 4-px frame, no padding) so it proves that
region and no other; measure the rect from the screenshot the box is drawn on.
The caption is burned in on a bar **below** the image, word-wrapped in
`project.language` — below, never above, so no pixel coordinate moves. Several
`--rect` flags box several regions under one caption.

Evidence layout (one folder per TC):
```
evd/<TICKET>/
├── manifest.md            # plain language: the requirement + the COVERAGE: block
│                         #   (security: TC_n | n/a — reason · accessibility: …) +
│                         #   the evd_index block (what was tested / which file is
│                         #   from which case) — regenerated, never hand-edited
├── <TICKET>_testcases.xlsx  # V7: the same record as a workbook (generated by
│                         #   xlsx_export.py; ignored by git — regenerate, never edit)
├── debate.md              # V6 cards (verifier + challenger)
├── data_prep/             # V3 runs, if any
└── TC_<n>_<what_it_proves>/     # the folder name IS the first line of the test plan
    ...
    │                     #   KIND: acceptance|boundary|whole-screen|write-readback|exploratory|security
...
Gate: `python3 .vteam/scripts/evd_check.py --evd evd/<TICKET>
...
Beside `COMMIT:` and `VERIFIED-AT:`, the report header carries two more anchors
the gate demands: `ENVIRONMENT: <name — url>` (local · dev · stg · prod — a bug
found on staging is not evidence about production) and `ORACLE: <what "correct"
was compared against>` (the spec shard, or `NONE` when nothing written decided).

Write `evd/<TICKET>/REPORT.md` — in `project.language`, black-box
...
`evd/<TICKET>/REPORT.bdd.md` — the verdict as Given/When/Then
scenarios a non-programmer scans in a minute (`docs/team/bdd-report.md`;
...
1. Finalize REPORT.md (challenger's readability fixes applied). Re-run
   `evd_check.py` — must be green. Then make the folder introduce itself and
   produce the spreadsheet, because most of the people this verdict is for will
   never open a markdown file:

   ```bash
   python3 .vteam/scripts/evd_index.py  --evd evd/<TICKET>            # index block → manifest.md
   python3 .vteam/scripts/xlsx_export.py --evd evd/<TICKET> --strict  # <TICKET>_testcases.xlsx
   ```

   The workbook is the same record laid out to ISO/IEC/IEEE 29119-3 — a Summary
   that answers *how many defects and how bad is the worst one* on the first
   screen, the cases as a test-case specification, the defects graded by
   severity, a requirement traceability matrix, and an index of every evidence
   file (headers in `project.language`). It **never invents a value**: a field the
   pack lacks prints NOT DECLARED, and `--strict` exits 1 naming each one — fill
   the pack and re-run. **Fix the pack, never the spreadsheet**: it is generated,
   and a hand-edited workbook is a workbook nobody can check against the evidence.
...
3a. **Attach QA images to the tracker — mandatory, with read-back** (images live
   ...
   `python3 .vteam/scripts/evd_check.py --evd evd/<TICKET> --attach <TICKET>`
   — uploads every TC image (attach the `<TICKET>_testcases.xlsx` alongside —
   it is the artefact a manager will actually open), re-reads the attachment
   list to confirm each filename, and writes a `## TRACKER ATTACHMENTS` section (name · md5 · url)
...
6. **A finding OUTSIDE the ticket's scope that deserves its own ticket → QA files
   ...
   [Expected]  per spec §x.y (docs/specs/<feature>.md)
   [Actual]  what happened
   [Severity]  Blocker / Critical / Major / Minor (roles/qa.md)
   [EVD]  evd/<TICKET>/TC_n/… (boxed image, db_verify if a write is involved)
   ```
...
7. **Close the learning loop** — append a lesson (+ INDEX line) to
   `docs/qa/knowledge-base.md`, even for FAIL/BLOCKED runs (or state "no new
...
- [ ] `evd_check.py --expect-tcs <N>` green; REPORT.md follows the template
      (with the COMMIT:, VERIFIED-AT:, ENVIRONMENT: and ORACLE: lines), jargon-free body
- [ ] Every TC has TITLE: (a behaviour sentence) + KIND:; the pack holds a
      `boundary` and a `whole-screen` case; folders are `TC_<n>_<what_it_proves>`;
      EXPECTED/ACTUAL are observable values, never a judgement word
- [ ] Root manifest carries the COVERAGE: block (security + accessibility each a
      case or a reasoned waiver) and a CURRENT evd_index block (`evd_index.py --check`)
- [ ] `xlsx_export.py --evd … --strict` green — `<TICKET>_testcases.xlsx` produced
      from the pack, attached to the ticket, never committed or hand-edited
- [ ] Every *_boxed.png hugs the exact region it proves, caption burned in below
